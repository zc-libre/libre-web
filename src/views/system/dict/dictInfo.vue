<template>
  <div>
    ...
    <div v-else>
      ...
      >
        <el-table-column label="所属字典">{{ query.name }}</el-table-column>
        <el-table-column prop="label" label="标签" width="80" />
        <el-table-column prop="value" label="值" width="60" />
...
<script>
  import crudDictInfo from '@/api/system/dictInfo'
  ...
  export default {
    ...
    cruds() {
      return [
        CRUD({
          ...
          crudMethod: { ...crudDictInfo },
          optShow: { add: true, edit: true, del: true, reset: false },
    ...
    mixins: [
      ...
      header(),
      form(function () {
        return Object.assign({ type: this.type }, defaultForm)
      }),
    ],
    data() {
      return {
