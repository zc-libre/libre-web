  ...
  export default {
    ...
    data() {
      return {
        ...
        permission: {
          add: ['admin', 'system:dept:add'],
          edit: ['admin', 'system:dept:edit'],
          del: ['admin', 'system:dept:del'],
        },
      }
    },
    methods: {
      ...
      getSupDepts(id) {
        crudDept.getDeptSuperior(id).then((res) => {
          this.depts = arrayToTree(res, { parentProperty: 'parentId' }).map(
            function (obj) {
      ...
      getDepts() {
        crudDept.getDepts({ enabled: 1 }).then((res) => {
          this.depts = arrayToTree(res.records, {
      ...
      loadDepts({ action, parentNode, callback }) {
        if (action === LOAD_CHILDREN_OPTIONS) {
          crudDept.getDepts({ enabled: 1, pid: parentNode.id }).then((res) => {
            parentNode.children = arrayToTree(res.records, {
              parentProperty: 'parentId',
            }).map(function (obj) {
              if (obj.hasChildren) {
                obj.children = null
